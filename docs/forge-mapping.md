...
- **GitLab:** cursor is the last seen settled-discussion max-note timestamp, ISO-8601 with `.NNN` fractional precision preserved. Fetch discussions, normalize each note's `created_at` to `.000Z` if no fractional is present, filter to threads where all notes have `created_at > cursor` (lex compare on the normalized strings), sorted by max note time. Same-second collisions are common enough that second-precision cursoring would drop sibling batches; fractional precision sidesteps it.
