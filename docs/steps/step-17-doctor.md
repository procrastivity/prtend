...
Resolve the active forge by calling `prtend_forge_detect` once and memoizing the result in a function-local `forge` value (then reuse that same value in `forge_cli_authed` / `forge_cli_version` — pass it as an argument to each helper rather than relying on a global). `prtend_forge_detect` already caches the resolved value into the **exported** `PRTEND_FORGE` variable (see `lib/prtend/prtend-forge-lib.bash:130-135`); read it back as `${PRTEND_FORGE:-}` (always guard with `:-` to survive `set -u`). Do **not** invent an underscore-prefixed alias — `PRTEND_FORGE` is the canonical name. On `github`, check `command -v gh` and capture `gh --version | head -n 1` for the message. On `gitlab`, mirror with `glab`. When forge detection itself fails (no `.git`, no remote, no `PRTEND_FORGE`), status is `fail` with `message:"no forge detected"`, `fixable=false`. When the binary isn't on PATH, status is `fail` with `message:"gh not found on PATH"` (or `glab`), `fixable=false`.
...
- Call `prtend_forge_pr_state "$pr"`. **Only a successful call (exit 0) whose `.state` field parses to one of `{closed, merged}` is treated as stale.** A non-zero exit, an empty body, a body that does not parse as JSON, or a body whose `.state` is anything else (including missing) is treated as **non-stale** and the file is left in place. Rationale (Codex P2 #1): `prtend_forge_pr_state` dispatches to the raw forge CLI and does not distinguish "PR not found (404)" from "transient API/network failure"; both surface as exit 1 today. Conservatively skipping the ambiguous case prevents `doctor --fix` from deleting an active subscription's state file because the user's wifi blinked. The cost is that genuine "PR was deleted on the forge" cases require manual `rm` — acceptable for v0; if a future forge-lib gains a not-found discriminator, doctor can promote that signal back to stale.
...
- **Stale detection requires positive closed/merged JSON.** A `prtend_forge_pr_state` call that exits non-zero, returns empty, or whose `.state` field is missing/other is treated as non-stale — never as "the PR was deleted, clean it up." Today the forge lib does not distinguish 404 from transient API/network errors (both come through as exit 1), so `--fix` would otherwise risk deleting state for an active subscription during a flaky network. A future forge-lib that exposes a "not found" discriminator can promote that signal back to stale; for v0 the conservative rule stands.
...
7. **Config malformed** — write a file containing a line outside the accepted grammar (e.g. a bare `&^%` line, or an item dash with no leading whitespace) at the resolved path. `config_readable` → `fail`, message has the form `<path>:<line-no>: malformed line`. Exit 1.
...
10. **Stale subscriptions — two stale, no `--fix`** — seed four state files; fake `prtend_forge_pr_state` returns `{"state":"open",…}` for PR 100, `{"state":"closed",…}` for 101, `{"state":"merged",…}` for 102, and **exits 1** for PR 103 (simulating a transient API error). `stale_subscriptions` → `warn` with `message:"2 state files for closed PRs (PR #101, #102)"`, `fixable=true`. Crucially, PR 103 is **not** in the stale list (positive-JSON-only rule). `summary.warn=1`, `fail=0`. Exit 0. All four state files still on disk.
11. **Stale subscriptions — two stale, `--fix`** — same fixture as case 10, run with `--fix`. After the fix pass, the row's `status` is `pass`; `fixed=[{check:"stale_subscriptions", action:"removed", details:[<path-101>, <path-102>]}]`. Only PRs 101 and 102 are removed from disk; PR 100 (`open`) and PR 103 (transient error) are untouched. Exit 0.
12. **Stale subscriptions — partial fix failure** — same fixture but `chmod 0500` the state dir before `--fix` so `rm` fails. The fix routine logs warnings to stderr; the row stays `warn`; `fixed=[{check:"stale_subscriptions", action:"removed", details:[]}]` (empty details — nothing was actually removed). Exit **0** — a surviving `warn` does not trip the fail-exit rule (only `fail` does). Per the contract: exit 1 only when `summary.fail > 0`.
