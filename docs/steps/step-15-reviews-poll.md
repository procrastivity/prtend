...
- Step 04 (`forge-read`) complete — `prtend_forge_reviews_since`, `prtend_forge_review_comments`, `prtend_forge_review_thread_notes` (new in this step — see "Files to create or modify"), and `prtend_forge_pr_state` are the four forge ops this subcommand uses, in that order per batch.
...
- `already_handled` is true when prtend's marker appears on a reply posted *after* the projected comment's `created_at` (NOT just anywhere in the thread). Computed by calling `prtend_forge_review_thread_notes "$pr" "$comment_id"` (new primitive — same shape as `review_thread_bodies` but per-note with `created_at`), filtering to notes with `created_at > comment.created_at`, and running each body through `prtend_note_is_handled`. The time-filter is what distinguishes a previously-replied request comment ("handled") from a fresh human follow-up in the same thread ("not yet handled" — `overview.md` § "Edge cases" item 11). `note-post` keeps using the simpler `review_thread_bodies` (it just needs any-marker-in-thread to refuse a double-post, no time semantics required).
...
### `lib/prtend/prtend-forge-lib.bash` — per-batch `resume_cursor`, two ordering fixes, one new primitive

Five changes in this file. All small, all required to honor the one-batch-per-blocking-call contract, sort-order correctness across submission races, and the `already_handled` time-filter semantic:
...
1. **Add a `resume_cursor` field to every batch.** Emitted by both `_prtend_forge_gh_reviews_since` and `_prtend_forge_gl_reviews_since`. It's the per-batch resume token — the cursor value the *next* `reviews_since` call should pass to skip past this batch alone:
   - GitHub: the compound pair `"<submitted_at>|<id>"` (see fix 2 below for why id alone is insufficient). `resume_cursor: "${submitted_at}|${id}"` next to `batch_id`.
...
2. **GitHub: cursor is the compound pair `(submitted_at, id)`, wire-encoded as `"<submitted_at>|<id>"`.** An id-only cursor is unrecoverable: GitHub assigns review ids at *draft creation*, not submission, so a draft with id=100 created before id=200 can be submitted *after* id=200 has already been processed; the next call would filter it out via `.id > 200` and silently lose its review comments forever. Filter via `(.submitted_at > $cur_ts) OR (.submitted_at == $cur_ts AND .id > $cur_id)` and sort by `(submitted_at, id)` ascending. The across-call `next_cursor` is the last batch's compound `resume_cursor`. Sorting by submission time is also the order downstream consumers care about (the watch loop sees reviews in the order reviewers submit them). The previous version of `../cli-contract.md` and `../forge-mapping.md` documented the cursor as the bare last review id, so a bare numeric cursor coming from an existing state file or an explicit `--cursor 200` call must be honored under the legacy id-only filter (`.id > $cur_id`) — otherwise old cursors silently re-emit every review. The next emission writes the compound form, self-healing the state.
...
5. **Add `prtend_forge_review_thread_notes <pr> <comment_id>`.** Same thread-discovery as `review_thread_bodies` (used by note-post for double-post prevention) but emits per-note objects `{notes: [{id, created_at, body}]}` instead of concatenated body strings. Required so reviews-poll can time-filter the marker check to "replies posted *after* this comment's created_at" — a fresh human follow-up in an already-replied thread must surface with `already_handled:false`, otherwise the skill (per `skill-prompts.md:121`) silently drops it. `note-post` keeps the simpler `review_thread_bodies` primitive — it just needs any-marker-in-thread for refusal, no time semantics.

4. **GitLab: surface follow-ups on already-emitted discussions.** A discussion whose original notes have all been emitted (cursor advanced past the previous max-note time) can still receive a fresh human follow-up — `overview.md` § "Edge cases" item 11 promises this surfaces as "a fresh comment in the next review batch." Filter `any($times > $cur)`, not `all($times > $cur)`: the latter drops the whole discussion the moment one old note exists. Compute the batch metadata (`submitted_at`, `author`, `comment_ids`) from the *newer-than-cursor* notes only, so the emitted event represents the new activity rather than the entire discussion history. The subcommand restricts its `review_comments` projection to the batch's `comment_ids` set so re-emissions surface only fresh notes. `_max_t` (the cursor advance target) is still `max(all_times)` so the next call doesn't re-emit the same notes.
